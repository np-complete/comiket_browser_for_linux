...
describe "CirclesController" do
  ...
  let!(:circles) { FactoryGirl.create_list(:circle, 60, block: blocks.first) }
  let!(:circles_2) { FactoryGirl.create_list(:circle, 60, block: blocks[2]) }
  ...
  context :block_id_given do
    ...
    context :other_prev_block do
      ...
      its(:cond) do
        should == {
          :prev => {page: 0, day: 1, block_id: blocks[0].id},
